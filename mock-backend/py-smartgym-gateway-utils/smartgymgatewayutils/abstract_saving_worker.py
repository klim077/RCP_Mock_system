...
class Abs_saving_worker(metaclass=abc.ABCMeta):
    ...
    def post_workout(self, stage_3_channel=create_exercise_channel):
        ...
        try:
            ...
            self.__send_redis_update(stage_3_channel=stage_3_channel)
        except Exception as e:
            # gwLogger.logger.error('post workout exception caught: {}'.format(e))
            logger.error('post workout exception caught: {}'.format(e))
            pass
    
    def __send_redis_update(self, stage_3_channel=create_exercise_channel):
        try:
            redis_payload_data = {
            ...
            logger.info(payload)
            numof_subs = r.publish(stage_3_channel, payload)
